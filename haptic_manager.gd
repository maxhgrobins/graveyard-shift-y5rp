...
var type = {
	...
	Vibration.BOW_FIRE:     [1.0, 1.0, 0.15]
}
...
func play(vibration: Vibration, hand: String = "both", delay: float = 0.0):
	...
	vibrate(v_data, hand, delay)

## can be called by creating new v_data
func vibrate(v_data: Array, hand: String = "both", delay: float = 0.0):
	if hand != "both" and hand != "right_hand" and hand != "left_hand":
		push_error(hand," is not a valid hand for vibration!")
	if hand == "both":
		vibrate(v_data, "right_hand", delay)
		vibrate(v_data, "left_hand", delay)
		return
